<script lang="ts">
  ...
  async function pingServer(hostname: string): Promise<PingResult> {
    const response = await fetch(serverPingUrl(hostname));
  ...
  const request = $derived(host === '' ? null : pingServer(host));
</script>
